...
@Injectable()
export class CanActivateGeneralService implements CanActivate{
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean | Observable<boolean> | Promise<boolean> {
    ...
    return new Promise((resolve, reject) => {
      this.globalService.getUser().then(user => {
        ...
        this.rolmenuService.getMenu(this.user.rol.id,'SOS_007').subscribe(permiso => {
          if(permiso){
            resolve(true);
          }
          else{
            this.UTILS.showMensajesSms('Señor Usuario.  No tiene permisos para ingresar a la opción de '+
              'Configuración del Sistema / Generales.');
